...
namespace WpfApp2
{
    class mutiplethread
    {
        private double strike, stockprice, rate, vol, time; int trial, step, core; double[,] threadmatrix;
        ...
        public double[] threadpractice(double strike, double stockprice, double rate, double vol, int step, int trial, int corenumber, double time, bool antithetic, bool cvmethod, bool calloption)
        {
            ...
            else//not cv method
            {
                if (calloption)
                {
                    double[] option = eco(strike, stockprice, rate, vol, step, 8 * trial, time, _matrix, 0);
                    //get options under different parameters
                    double o1 = option.Average();
                    double o2 = eco(strike, stockprice + deltas, rate, vol, step, 8 * trial, time, _matrix, 1).Average();
                    double o3 = eco(strike, stockprice - deltas, rate, vol, step, 8 * trial, time, _matrix, 2).Average();
                    double o4 = eco(strike, stockprice, rate, vol + deltavol, step, 8 * trial, time, _matrix, 3).Average();
                    double o5 = eco(strike, stockprice, rate + deltar, vol, step, 8 * trial, time, _matrix, 4).Average();
                    double o6 = eco(strike, stockprice, rate, vol, step, 8 * trial, time + deltat, _matrix, 5).Average();
                    double o7 = eco(strike, stockprice, rate, vol - deltavol, step, 8 * trial, time, _matrix, 6).Average();
                    double o8 = eco(strike, stockprice, rate - deltar, vol, step, 8 * trial, time, _matrix, 7).Average();
                    double delta = (o2 - o3) / (2 * deltas); //get delta
                    double gamma = (o2 + o3 - o1 * 2) / (deltas * deltas); //get gamma
                    double vega = (o4 - o7) / (2 * deltavol);//get vega
                    double rho = (o5 - o8) / (2 * deltar);//get rho
                ...
                {
                    //get options under different parameters
                    double[] option = epo(strike, stockprice, rate, vol, step, 8 * trial, time, _matrix, 0);
                    double o1 = option.Average();
                    double o2 = epo(strike, stockprice + deltas, rate, vol, step, 8 * trial, time, _matrix, 1).Average();
                    double o3 = epo(strike, stockprice - deltas, rate, vol, step, 8 * trial, time, _matrix, 2).Average();
                    double o4 = epo(strike, stockprice, rate, vol + deltavol, step, 8 * trial, time, _matrix, 3).Average();
                    double o5 = epo(strike, stockprice, rate + deltar, vol, step, 8 * trial, time, _matrix, 4).Average();
                    double o6 = epo(strike, stockprice, rate, vol, step, 8 * trial, time + deltat, _matrix, 5).Average();
                    double o7 = epo(strike, stockprice, rate, vol - deltavol, step, 8 * trial, time, _matrix, 6).Average();
                    double o8 = epo(strike, stockprice, rate - deltar, vol, step, 8 * trial, time, _matrix, 7).Average();
                    double delta = (o2 - o3) / (2 * deltas); //get delta
                    double gamma = (o2 + o3 - o1 * 2) / (deltas * deltas); //get gamma
                    double vega = (o4 - o7) / (2 * deltavol);//get vega
                    double rho = (o5 - o8) / (2 * deltar);//get rho
        ...
        static double antise(double[] option, int corenumber) //get standard error using antithetic method
        {
        ...
        static double[] eco(double strike, double stockprice, double rate, double vol, int step, int trial, double time, double[,] matrix, int position)
        {
        ...
        static double[] epo(double strike, double stockprice, double rate, double vol, int step, int trial, double time, double[,] matrix, int position)
        {
        ...
        //use CVeco and CVepo method to get call and put in control variate
        public static double[] CVmethod(double strike, double stockprice, double rate, double vol, int step, int trial, double time, double[,] pricematrix, bool calloption)
        {
            double deltas = 0.001 * stockprice;
            double deltavol = 0.001 * vol;
            double deltar = 0.001 * rate;
            ...
            //public static double[] CVepo(double strike, double stockprice, double rate, double vol, int step, int trial, double time, double[,] randommatrix,int position)
            //{
            //    double dt = time / step; double t;
            //    CDFmethod cdf = new CDFmethod();
            //    double[] ct = new double[trial/8];//define a list to save option price
            //    int k = 0;
            //    for (int i = position; i < trial; i=i+8)
            //    {
            ...
            //        {
            //            t = dt * (step - j);
            //            double d1 = (Math.Log(randommatrix[i, j - 1] / strike) + (rate + Math.Pow(vol, 2) / 2) * t) / (vol * Math.Pow(t, 0.5));
            //            double delta = cdf.CumDensity(d1) - 1;
            //            cv = cv + delta * (randommatrix[i, j] - randommatrix[i, j - 1] * Math.Exp(rate * dt));
            //        }
            //        ct[k] = (Math.Max(0, -randommatrix[i, step - 1] + strike) - cv) * Math.Exp(-rate * time);
            //        k = k + 1;
