...
namespace WpfApp2
{
    class multiplethread_range:multiplethread
    {
        public override double[] call(double strike, double stockprice, double rate, double vol, int step, int trial, double time, double[,] matrix, int position)
        {
            ...
            for (int i = position; i < trial; i = i + 8)
            {
                ...
                { if (maxprice[j] < matrix[i, k]) maxprice[j] = matrix[i, k];if (minprice[j] > matrix[i, k]) minprice[j] = matrix[i, k]; }
                call[j] =(maxprice[j] -minprice[j]) * Math.Exp(-rate * time); //compute option value at t=0 in every simulation
                j = j + 1;
            }
            return call;
        }
        public override double[] put(double strike, double stockprice, double rate, double vol, int step, int trial, double time, double[,] matrix, int position)
        {
            ...
            for (int i = position; i < trial; i = i + 8)
            {
                ...
                for (int k = 0; k < step; k++)
                { if (maxprice[j] < matrix[i, k]) maxprice[j] = matrix[i, k]; if (minprice[j] > matrix[i, k]) minprice[j] = matrix[i, k]; }
                put[j] =(maxprice[j]-minprice[j]) * Math.Exp(-rate * time); //compute option value at t=0 in every simulation
                j = j + 1;
            }
            return put;
        }
        ...
        public override double[] CVmethod(double strike, double stockprice, double rate, double vol, int step, int trial, double time, double[,] pricematrix, bool calloption)
        {
            ...
            for (int i = 0; i < 8 * trial; i = i + 8)
            {
                ...
                for (int j = 1; j < step; j++)
                {
                    ...
                    for (int k = i; k < 8 + i; k++) { if (maxprice[k] < pricematrix[k, j]) maxprice[k] = pricematrix[k, j]; if (minprice[k] > pricematrix[k, j]) minprice[k] = pricematrix[k, j]; }
