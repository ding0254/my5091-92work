...
def bisection(s,k,T,r,optprice,size,optype):
    ...
    if optype:
        ...
        F = lambda x: (putprice(s, k, r, x, T) - optprice)  #x is a variable for vol
        while sum((np.abs(vol1-vol2)<1e-7))<size:    #define the accuracy
            vol3=0.5*(vol1+vol2)              #the middle value of the lower and upper boundary
            a=F(vol3)
            lowbools=(a>0)            
...
def newton(s,k,T,r,optprice,size,optype):   #the Newton method
    ...
    if optype:
        ...
        F = lambda x: (putprice(s, k, r, x, T) - optprice)
        while sum(np.abs(deltax)<1e-7) <size and iter<10:   #to make sure the accuracy and not to go into infinite loop
            x1=x0+(0-F(x0))/df(x0)     #use the Newton method formula 
            deltax=x1-x0
            x0=x1
...
def output(S,K,T,r,optprice,size,optype):    #output function :optprice means option price and size means the number of price                
     bisectionvol = bisection(S, K, T, r, optprice,size,optype) #the value of bisction method
     newtonvol=newton(S, K, T, r, optprice,size,optype)       #the value of Newton method
